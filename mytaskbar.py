# Manca:
# AGGIUNGERE: rendere risposizionabili icone quicklaunch, e non fare il rearrange automatico
# AGGIUNGERE: ciclare su tutte le finestre massimizzate in modo da massimizzarle ogni volta che si ridimensiona la barra
# AGGIUNGERE: Evidenziare nel menu in quale schermo ci troviamo
# AGGIUNGERE: Creazione automatica di link a siti web (ora va editato a mano il json)
# AGGIUGERE: hover su tasti quicklaunch per mostrare percorso
# ERRORE: PSPad viene mostrato senza icona
# ERRORE: Le cartelle appaiono senza icone
# ERRORE: Lo sfondo dei tasti sembra diviso in due
# ERRORE: Continuano a comparire app fantasma (impostazioni e altro)
...
class ApplicationIconViewer:
    def __init__(self, root):
        ...
        self.dragged_item = None
        self.is_dragging = False  # Variabile di stato per tracciare il trascinamento
...
######################
    ...
    def on_drag_motion(self, event):
        """Gestisce il movimento del pulsante durante il trascinamento"""
    ...
    def on_drag_end(self, event):
        """Termina il trascinamento e posiziona il pulsante nella nuova posizione"""
        if self.dragged_item:
            widget = self.dragged_item
            widget.grid_forget()  # Rimuovi il pulsante dalla griglia
            x = widget.winfo_x()
            y = widget.winfo_y()
            # Trova la nuova posizione nella griglia
            new_col = int(x // (widget.winfo_width() + 10))  # +10 per il padding
            new_row = int(y // (widget.winfo_height() + 10))  # +10 per il padding
            widget.grid(row=new_row, column=new_col, padx=5, pady=5)  # Riposiziona il pulsante nella griglia
            # Aggiorna i dati del pulsante
            for button_data in self.quicklaunch_buttons:
                if button_data['name'] == widget.cget('text'):
                    button_data['row'] = new_row
                    button_data['col'] = new_col
                    break
    ...
    def add_quicklaunch_button(self, button_name, file_path, arg_path, icon_path, row=None, col=None):
        ...
        try:
            ...
            if is_directory(file_path):
                ...
                command = lambda: self.launch_executable(file_path, arg_path2)
...
################# debug drag
            ...
            def launch_application(self, command):
                """Lancia l'applicazione associata al pulsante"""
                if self.drag_timer:
            ...
            def on_button_press(event):
                """Gestisce la pressione del pulsante per il drag and drop"""
                print("cliccato...")
                self.dragged_item = event.widget
                print("   imposto self.dragged_item = event.widget")
                self.dragged_item.startX = event.x
                self.dragged_item.startY = event.y
                self.is_dragging = False  # Inizialmente non è un trascinamento
                print("   self.is_dragging = False  # Inizialmente non è un trascinamento")

            def on_button_motion(event):
                """Gestisce il movimento del mouse durante il trascinamento"""
                print("muovo")
                if self.dragged_item:
                    print("   self.dragged_item")
                    # Se il mouse si è mosso significativamente, è un trascinamento
                    if abs(event.x - self.dragged_item.startX) > 5 or abs(event.y - self.dragged_item.startY) > 5:
                        print("Ok imposto come dragging")
            ...
            def on_button_release(event):
                """Gestisce il rilascio del pulsante per il lancio dell'applicazione o il posizionamento dopo il drag"""
                print ("release")
                if self.dragged_item:
                    widget = self.dragged_item
                    if not self.is_dragging:
                        if widget == event.widget:
                            # Lancia l'applicazione solo se non è avvenuto un trascinamento
                            print("lancio")
                            command()
                    else:
                        print("Fine draggin, valuto riposiz.")
                        # Calcola la nuova posizione nella griglia
                        x = widget.winfo_x()
                        y = widget.winfo_y()
                        new_col = int(x // (widget.winfo_width() + 10))  # +10 per il padding
                        new_row = int(y // (widget.winfo_height() + 10))  # +10 per il padding
                        print(f"{new_col},{new_row}")

                        # Verifica se la nuova posizione è già occupata
                        position_occupied = False
                        for button_data in self.quicklaunch_buttons:
                            if button_data['row'] == new_row and button_data['col'] == new_col:
                                position_occupied = True
                                break

                        print(f"{position_occupied}")

                        if not position_occupied:
                            # Aggiorna la posizione del pulsante nella griglia
                            widget.grid(row=new_row, column=new_col, padx=5, pady=5)
                            ...
                            # Se la posizione è già occupata, ripristina la posizione originale
                            original_row = None
                            original_col = None
                            for button_data in self.quicklaunch_buttons:
                                if button_data['name'] == widget.cget('text'):
                                    original_row = button_data['row']
                                    original_col = button_data['col']
                                    break

                            if original_row is not None and original_col is not None:
                                widget.grid(row=original_row, column=original_col, padx=5, pady=5)
...
#################
            ...
            # Se row e col non sono specificati, usa grid_position per determinare la posizione
            if row is None or col is None:
                row = self.grid_position // MAXCOLS
                col = self.grid_position % MAXCOLS
                self.grid_position += 1
            else:
                # Se row e col sono specificati, aggiorna grid_position per evitare sovrapposizioni
                self.grid_position = max(self.grid_position, row * MAXCOLS + col + 1)
            ...
            self.quicklaunch_buttons.append(button_data)
            self.save_quicklaunch_data()
            return button
    ...
    def save_quicklaunch_data(self):
        ...
        try:
            # Ordina i pulsanti per riga e colonna
            sorted_buttons = sorted(self.quicklaunch_buttons, key=lambda x: (x['row'], x['col']))

            # Salva in file JSON
            config_file = "quicklaunch_config.json"
            with open(config_file, 'w', encoding='utf-8') as f:
                json.dump(sorted_buttons, f, indent=2, ensure_ascii=False)
        ...
        try:
            ...
            for data in quicklaunch_data:
                try:
                    ...
                    button = self.add_quicklaunch_button(button_name, file_path, arg_path, icon_path, row, col)
                    if button:
                        loaded_count += 1
